            ...
            <div>
                <span class="inline-block text-gray-400 text-sm text-clip ">
                    @if ($app->publisher->url)
                        <a class="hover:text-gray-600" href="{{$app->publisher->url}}" target="_blank" title="Opens in a new tab">{{$app->publisher->name}}</a>
                    @else
                        {{$app->publisher->name}}
                    @endif
                </span>
            ...
            <div class="text-gray-400 text-sm">
                @if($app->categories->count())
                    @foreach($app->categories as $cat)
                        <a class="hover:text-gray-600" href="{{route('categories.show', $cat->slug)}}" title="Goto {{$cat->name}}">{{$cat->name}}</a>
                    @endforeach
                @endif
            </div>
        @endif
